...
import { BLOG_POSTS, default as FlexilogicPortfolio } from "./FlexilogicPortfolio";
import { useSEO } from "./useSEO";

/* ── Lazy-load each blog article for code-splitting ─────────── */
const EduverseArticle = lazy(() =>
  import("./blog/building-school-management-systems-zimbabwe")
);
const AcademyProCaseStudy = lazy(() =>
  import("./blog/building-school-management-systems-zimbabwe")
);
...
const WebDevArticle = lazy(() =>
  import("./blog/web-platforms-zimbabwe")
);
const MobileAppsArticle = lazy(() =>
  import("./blog/mobile-apps-zimbabwe")
);
const CloudDevOpsArticle = lazy(() =>
  import("./blog/cloud-devops-africa")
);
const AIAutomationArticle = lazy(() =>
  import("./blog/ai-automation-africa")
);
const CybersecurityArticle = lazy(() =>
  import("./blog/cybersecurity-zimbabwe")
);
const DataAnalyticsArticle = lazy(() =>
  import("./blog/data-analytics-africa")
);
const DigitalMarketingArticle = lazy(() =>
  import("./blog/digital-marketing-roi-zimbabwe-website-growth-engine")
);

/* ── Register your posts here — slug MUST match ARTICLE.slug
   inside that file, or the sitemap/canonical URLs will lie ─── */
const blogCompany = [
  { slug: "building-school-management-system-zimbabwe", component: EduverseArticle },
  { slug: "academypro-school-management-case-study", component: AcademyProCaseStudy },
  ...
  { slug: "web-platforms-zimbabwe", component: WebDevArticle },
  ...
  { slug: "digital-marketing-roi-zimbabwe-website-growth-engine", component: DigitalMarketingArticle },
];
...
function BlogArticleRouter() {
  ...
  const match = blogCompany.find((r) => r.slug === slug);

  if (!match) return <Navigate to="/blog" replace />; // was "/" — send unknown slugs to the blog index, not the homepage
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8F6F1", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{
        ...
        background: "rgba(248,246,241,0.95)", backdropFilter: "blur(16px)",
        borderBottom: "1px solid #E4E1D9", padding: "0 48px", height: 64,
        display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <button onClick={() => navigate("/")} style={{
          display: "flex", alignItems: "center", gap: 8, background: "none",
          border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#6B7592",
        }}>
        ...
        <button onClick={() => navigate("/#contact")} className="btn-shimmer" style={{
          fontWeight: 700, fontSize: 12, color: "#0B1221", border: "none",
      ...
      <div style={{ padding: "64px 48px 48px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{
          display: "inline-flex", alignItems: "center", gap: 6, background: "#FDF3E0",
          border: "1px solid rgba(200,146,42,0.25)", color: "#C8922A", fontSize: 9,
        ...
        <h1 className="serif" style={{
          fontSize: "clamp(32px,5vw,56px)", fontStyle: "italic", color: "#0B1221",
      ...
      <div style={{
        maxWidth: 1100, margin: "0 auto", padding: "0 48px 96px",
        display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20,
      }}>
        {BLOG_POSTS.map((post) => (
          <div key={post.slug} className="all-card" onClick={() => navigate(`/blog/${post.slug}`)} style={{
            background: "#fff", border: "1px solid #E4E1D9", borderRadius: 24,
            overflow: "hidden", display: "flex", flexDirection: "column",
          }}>
            <div style={{
              background: post.coverBg, aspectRatio: "16/9", display: "flex",
              ...
              <div style={{ position: "absolute", top: 16, left: 20 }}>
                <span className={post.categoryColor} style={{
            ...
            <div style={{ padding: 24, display: "flex", flexDirection: "column", flex: 1 }}>
              <div style={{
                ...
                color: "#6B7592", fontWeight: 600, marginBottom: 12,
              }}>
                <span>{post.date}</span>
                <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#C8922A", display: "inline-block" }} />
                <span>{post.readTime}</span>
              </div>
              <h3 className="serif" style={{ fontSize: 18, fontStyle: "italic", color: "#0B1221", lineHeight: 1.35, margin: "0 0 12px", flex: 1 }}>
                {post.title}
              </h3>
              <p style={{ fontSize: 13, color: "#6B7592", lineHeight: 1.72, margin: "0 0 20px" }}>{post.excerpt}</p>
              <div style={{
                display: "flex", alignItems: "center", gap: 6, color: "#C8922A",
                fontWeight: 700, fontSize: 12, paddingTop: 16, borderTop: "1px solid #E4E1D9",
              }}>
                Read Article
