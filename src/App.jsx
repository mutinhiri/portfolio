...
/* ─────────────────────────────────────────────
   KEYFRAME ANIMATIONS — only things Tailwind
   can't express natively go here
───────────────────────────────────────────── */
const GlobalStyles = () => (
  <style>{`
    ...
    .cf-input.err {
      border-color: #ff7070;
      box-shadow: 0 0 0 3px rgba(255,112,112,0.10);
    }
  ...
  <div className="w-full h-full bg-gradient-to-br from-[#0A1628] to-[#0D2137] p-4 flex flex-col gap-2.5 font-sans">
    ...
    <div className="flex gap-2.5 flex-1 min-h-0">
      ...
      <div className="flex-1 flex flex-col gap-2">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {[
            { label: "Subjects", val: "24", sub: "All levels", color: "text-[#00CFFF]" },
            { label: "Resources", val: "3,400+", sub: "PDFs & videos", color: "text-[#FFD166]" },
            { label: "Students", val: "12k+", sub: "Active learners", color: "text-[#06D6A0]" },
            { label: "Access", val: "$12", sub: "Full year / all", color: "text-[#FF6B9D]" },
          ].map(s => (
            <div key={s.label} className="bg-white/[0.04] border border-white/[0.06] rounded-lg p-2">
...
const PROJECTS = [
  {
    id: "sms", featured: true,
    title: "Eduverse School Management System",
    desc: "A full-featured school administration platform for Zimbabwean secondary schools. Handles student enrollment, grade tracking, fee management, attendance, timetabling, and parent communication — unified in one dashboard.",
    badges: [{ label: "New", cls: "bg-[#00C896]/15 text-[#00C896]" }, { label: "Live", cls: "bg-[#C8922A]/15 text-[#C8922A]" }, { label: "Web App", cls: "bg-[#7C9FFF]/15 text-[#7C9FFF]" }],
    tech: ["React", "Node.js", "PostgreSQL", "Redis", "AWS S3", "Twilio SMS"],
    mockup: <SMSMockup />,
  },

  {
    id: "paygo", title: "PayGo Mobile",
    desc: "Cross-border mobile payment app for informal traders across Southern Africa.",
    badges: [{ label: "Mobile", cls: "bg-[#A78BFA]/15 text-[#A78BFA]" }, { label: "AI", cls: "bg-orange-400/15 text-orange-400" }],
    tech: ["Flutter", "Firebase", "Node.js"],
    mockup: <div className="w-full h-full bg-gradient-to-br from-[#0d1b2a] to-[#1a2f4a] flex flex-col items-center justify-center gap-2"><span className="text-4xl opacity-60">📲</span><span className="text-[10px] font-bold tracking-widest text-white/40 uppercase">PayGo Mobile</span></div>,
  },

  {
    id: "agrilink", title: "AgriLink Portal",
    desc: "Connecting smallholder farmers to markets, weather data, and financing.",
    badges: [{ label: "Web", cls: "bg-[#7C9FFF]/15 text-[#7C9FFF]" }],
    tech: ["Vue.js", "Django", "PostgreSQL"],
    mockup: <div className="w-full h-full bg-gradient-to-br from-[#0a1f0a] to-[#1a3a1a] flex flex-col items-center justify-center gap-2"><span className="text-4xl opacity-60">🌾</span><span className="text-[10px] font-bold tracking-widest text-white/40 uppercase">AgriLink Portal</span></div>,
  },
  {
    id: "eduvest", title: "Zim Learner Spot", featured: true,
    desc: "Impact investment platform connecting education-focused startups with African investors.",
    badges: [{ label: "Live", cls: "bg-[#C8922A]/15 text-[#C8922A]" }, { label: "Web", cls: "bg-[#7C9FFF]/15 text-[#7C9FFF]" }],
  ...
  return (
    <div className="modal-overlay fixed inset-0 z-[999] flex items-center justify-center p-6 bg-[#0B1221]/92 backdrop-blur-2xl" onClick={onClose}>
      <div className="modal-panel relative w-full max-w-4xl" onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="absolute -top-11 right-0 flex items-center gap-2 text-white/50 hover:text-white text-[13px] font-semibold transition-colors">
          Close <CloseIcon />
        </button>
        <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-[#080F1E]" style={{ aspectRatio: "16/9" }}>
          <div className="w-full h-full flex flex-col items-center justify-center gap-6 bg-gradient-to-br from-[#0B1221] via-[#131D35] to-[#1B2847]">
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <svg className="w-96 h-96 opacity-[0.04]" viewBox="0 0 300 300">
                <circle cx="150" cy="150" r="120" fill="none" stroke="#C8922A" strokeWidth="1" strokeDasharray="6 4" />
                <circle cx="150" cy="150" r="80" fill="none" stroke="#C8922A" strokeWidth="1" />
                {[0, 45, 90, 135, 180, 225, 270, 315].map((a, i) => (
                  <circle key={i} cx={150 + 80 * Math.cos(a * Math.PI / 180)} cy={150 + 80 * Math.sin(a * Math.PI / 180)} r="4" fill="#C8922A" />
                ))}
              </svg>
            </div>
            ...
            <div className="text-center z-10">
              ...
              <p className="text-white/40 text-[13px] max-w-sm text-center leading-relaxed">
                Add your video URL or file path to{" "}
                <code className="text-[#C8922A] text-xs bg-white/[0.06] px-1.5 py-0.5 rounded">ShowreelModal</code>{" "}
                inside this component to go live instantly.
              </p>
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#C8922A]/30 to-transparent" />
          </div>
        </div>
...
function StepCard({ step, idx }) {
  ...
  return (
    <div className="relative flex items-center min-h-[220px] mb-14">
      ...
      <div className="pulse-node absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-[34px] h-[34px] rounded-full bg-white border-2 border-[#C8922A] flex items-center justify-center"
        style={{ boxShadow: "0 0 0 7px rgba(200,146,42,0.12),0 4px 14px rgba(11,18,33,0.12)" }}>
        <div className="w-[11px] h-[11px] rounded-full bg-[#C8922A]" />
      </div>
      <div ref={ref}
        className={`rv ${isLeft ? "rl" : "rr"} ${vis ? "vis" : ""} relative overflow-hidden bg-white border border-[#E4E1D9] rounded-[20px] p-7 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:border-[#C8922A]/30 ${isLeft ? "mr-auto" : "ml-auto"}`}
        style={{ width: "calc(50% - 46px)", transitionDelay: `${idx * 0.08}s` }}>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {step.tags.map(t => <span key={t} className="text-[10px] font-bold tracking-[.06em] px-2.5 py-0.5 rounded-full uppercase bg-[#FDF3E0] text-[#C8922A] border border-[#C8922A]/20">{t}</span>)}
        </div>
...
function ProjectCard({ project }) {
  ...
  return (
    <div ref={ref}
      className={`rv ${vis ? "vis" : ""} group bg-white border border-[#E4E1D9] rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:border-[#C8922A]/30 ${project.featured ? "col-span-2" : ""}`}>
      <div className={`w-full overflow-hidden bg-[#0B1221] ${project.featured ? "aspect-[21/9]" : "aspect-video"}`}>
        {project.mockup}
      </div>
      <div className="p-6">
        <div className="flex gap-1.5 flex-wrap mb-3">
          {project.badges.map(b => <span key={b.label} className={`text-[9px] font-extrabold tracking-[.1em] px-2.5 py-0.5 rounded-full uppercase ${b.cls}`}>{b.label}</span>)}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 pt-4 border-t border-[#E4E1D9] mb-3">
          {project.tech.map(t => <span key={t} className="text-[10px] font-semibold text-[#6B7592] bg-[#F8F6F1] border border-[#E4E1D9] px-2.5 py-0.5 rounded-full">{t}</span>)}
        </div>
        <div className="flex items-center gap-1.5 text-[12px] font-bold text-[#C8922A] mt-2 cursor-pointer">
          View Case Study <Arr sz={12} />
        </div>
      </div>
    </div>
  );
...
/* ═══════════════════════════════════
   CONTACT SECTION
   Wire up handleSubmit to your backend:
   - EmailJS:   emailjs.send("SVC_ID","TPL_ID", form, "PUB_KEY")
   - Formspree: fetch("https://formspree.io/f/YOUR_ID",{method:"POST",body:JSON.stringify(form)})
   - Own API:   fetch("/api/contact",{method:"POST",body:JSON.stringify(form)})
═══════════════════════════════════ */
function ContactSection() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.errors) {
          setErrs({ submit: data.errors.map(err => err.message).join(", ") });
        } else {
          setErrs({ submit: "Something went wrong. Please try again." });
        }
      }
    } catch {
      setErrs({ submit: "Network error — please check your connection and try again." });
    } finally {
      setLd(false);
    }
  };
  ...
  const INFO = [
    { icon: "📧", label: "Email", value: "flexilogicafrica@gmail.com" },
    { icon: "📞", label: "Phone", value: "+263 77 000 0000" },
    { icon: "📍", label: "Location", value: "Harare, Zimbabwe" },
    { icon: "⚡", label: "Response", value: "Within 24 hours" },
  ];

  return (
    <section id="contact" className="relative overflow-hidden py-24 px-12"
      style={{ background: "linear-gradient(160deg,#0B1221 0%,#131D35 60%,#1B2847 100%)" }}>
      <div className="absolute bottom-[-8%] right-[-4%] w-[320px] h-[320px] opacity-[0.05] pointer-events-none">
        <svg width="320" height="320" viewBox="0 0 320 320">
          <circle cx="160" cy="160" r="120" fill="none" stroke="#C8922A" strokeWidth="1" strokeDasharray="8 5" />
          <circle cx="160" cy="160" r="65" fill="none" stroke="#C8922A" strokeWidth="1" />
          {[0, 45, 90, 135, 180, 225, 270, 315].map((a, i) => <circle key={i} cx={160 + 65 * Math.cos(a * Math.PI / 180)} cy={160 + 65 * Math.sin(a * Math.PI / 180)} r="3" fill="#C8922A" />)}
        </svg>
      </div>
      ...
      <div ref={sRef} className={`rv ${sVis ? "vis" : ""} relative z-10 max-w-[1100px] mx-auto`}>
        ...
        {/* Header */}
        <div className="mb-12">
          <div className="inline-flex items-center gap-2 bg-[#C8922A]/10 border border-[#C8922A]/25 text-[#C8922A] text-[10px] font-bold tracking-[.12em] px-3.5 py-1.5 rounded-full mb-4 uppercase">
            <span className="w-1.5 h-1.5 rounded-full bg-[#C8922A] inline-block" />
            Start a Conversation
          </div>
          <h2 className="serif italic text-white leading-[.95] mb-4" style={{ fontSize: "clamp(34px,5vw,60px)" }}>
            Let's Make <span className="text-[#C8922A]">Something</span><br />Legendary.
          </h2>
          <p className="text-[15px] text-white/50 max-w-[480px] leading-[1.78]">
            Tell us what you're building. We review every submission personally and reply within 24 hours.
          </p>
        </div>
        ...
        <div className="grid gap-8" style={{ gridTemplateColumns: "1fr 300px" }}>
          ...
          <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-8 backdrop-blur-sm">
            {sent ? (
              <div className="success-pop flex flex-col items-center justify-center text-center py-14 gap-5">
                <div className="w-16 h-16 rounded-full bg-[#00C896]/15 border border-[#00C896]/35 flex items-center justify-center text-[#00C896]">
                  <CheckCircle />
                </div>
                <div>
                  <h3 className="serif italic text-white text-[26px] mb-2">Message sent!</h3>
                  <p className="text-white/50 text-[14px] leading-relaxed max-w-xs">
                    Thanks for reaching out. We'll be in touch within{" "}
                    <span className="text-[#C8922A] font-semibold">24 hours</span>.
                  </p>
                </div>
                <button onClick={() => { setSent(false); setForm(BLANK); }}
                  className="mt-1 text-[12px] font-bold text-[#C8922A] border border-[#C8922A]/30 px-5 py-2 rounded-lg hover:bg-[#C8922A]/10 transition-all cursor-pointer bg-transparent">
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className={`cf-input${errs.name ? " err" : ""}`} placeholder="Tinashe Banda"
                      value={form.name} onChange={e => set("name", e.target.value)} />
                    {errs.name && <p className="cf-error">{errs.name}</p>}
                  </div>
                  <div>
                    <label className="cf-label">Company / Organisation</label>
                    <input className="cf-input" placeholder="ZimTech Holdings"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="email" className={`cf-input${errs.email ? " err" : ""}`} placeholder="you@company.co.zw"
                      value={form.email} onChange={e => set("email", e.target.value)} />
                    {errs.email && <p className="cf-error">{errs.email}</p>}
                  </div>
                  <div>
                    <label className="cf-label">Phone / WhatsApp</label>
                    <input className="cf-input" placeholder="+263 77 000 0000"
                ...
                <div>
                  ...
                  <select className={`cf-input${errs.service ? " err" : ""}`}
                    value={form.service} onChange={e => set("service", e.target.value)}>
                    <option value="">Select a service…</option>
                    {SVCS.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  {errs.service && <p className="cf-error">{errs.service}</p>}
                </div>

                <div className="grid grid-cols-2 gap-4">
                  {/* <div>
                    <label className="cf-label">Budget Range</label>
                    <select className="cf-input" value={form.budget} onChange={e => set("budget", e.target.value)}>
                  ...
                  <div>
                    ...
                    <select className="cf-input" value={form.timeline} onChange={e => set("timeline", e.target.value)}>
                ...
                <div>
                  <label className="cf-label">Tell Us About Your Project <span>*</span></label>
                  <textarea className={`cf-input${errs.message ? " err" : ""}`}
                    placeholder="Describe your idea, the problem it solves, and any technical details…"
                    value={form.message} onChange={e => set("message", e.target.value)} />
                  {errs.message && <p className="cf-error">{errs.message}</p>}
                </div>
                {errs.submit && (
                  <p className="text-center text-[12px] text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-2">
                    {errs.submit}
                  </p>
                )}

                <button onClick={handleSubmit} disabled={loading}
                  className="btn-shimmer w-full font-bold text-[15px] text-[#0B1221] border-none rounded-xl py-3.5 cursor-pointer flex items-center justify-center gap-2.5 disabled:opacity-60 disabled:cursor-not-allowed mt-1">
                  {loading ? (
                    <>
                      <svg className="spin-loader" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                        <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" />
                      </svg>
                      Sending…
                    </>
                  ) : (
                    <> Send Message <Arr sz={15} /> </>
                  )}
                </button>
          ...
          <div className="flex flex-col gap-4">
            {INFO.map(c => (
              <div key={c.label}
                className="flex items-center gap-3.5 bg-white/[0.04] border border-white/[0.07] rounded-xl px-4 py-3.5 hover:border-[#C8922A]/30 hover:bg-[#C8922A]/[0.04] transition-all">
                <div className="w-10 h-10 rounded-xl bg-[#C8922A]/15 flex items-center justify-center text-lg flex-shrink-0">{c.icon}</div>
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-[.08em] text-white/35 mb-0.5">{c.label}</div>
                  <div className="text-[13px] font-semibold text-white/80">{c.value}</div>
                </div>
              </div>
            ))}
            ...
            <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-5 mt-1">
              ...
              {["Global standards", "Fixed-scope or retainer options", "Dedicated PM on every project", "Post-launch support included", "Transparent pricing, no surprises"].map(item => (
                <div key={item} className="flex items-start gap-2.5 mb-3 last:mb-0">
                  <div className="w-4 h-4 rounded-full bg-[#C8922A]/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#C8922A" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                  </div>
            ...
            <div className="bg-white rounded-xl p-4 border-l-[3px] border-[#C8922A]">
              <p className="serif italic text-[13px] text-[#0B1221] leading-snug mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-[11px] font-bold text-[#0B1221]">Chiedza M.</div>
                  <div className="text-[10px] text-[#6B7592]">CEO, EduVest Africa</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-[100] px-12 h-16 flex items-center justify-between transition-all duration-300 ${scrolled ? "bg-[#0B1221]/95 backdrop-blur-[18px] border-b border-white/[0.07]" : "bg-transparent"}`}>
        ...
        <div className="flex items-center gap-0.5">
          {["Services", "Process", "Work", "Contact"].map(l => (
      ...
      <section id="hero" className="relative min-h-screen flex items-center overflow-hidden px-12 pt-24 pb-20" style={{ background: "linear-gradient(160deg,#0B1221 0%,#131D35 55%,#1B2847 100%)" }}>
        <div className="absolute top-0 right-0 w-[480px] h-[480px] opacity-[0.06] pointer-events-none">
          <svg width="480" height="480" viewBox="0 0 480 480">
            ...
            {[0, 60, 120, 180, 240, 300].map((a, i) => <circle key={i} cx={240 + 120 * Math.cos(a * Math.PI / 180)} cy={240 + 120 * Math.sin(a * Math.PI / 180)} r="5" fill="#C8922A" />)}
          </svg>
        </div>
          ...
          <div className="grid grid-cols-2 gap-20 items-center">
            <div>
              ...
              <p className="afu text-[15px] leading-[1.78] text-white/60 max-w-[430px] mb-9" style={{ animationDelay: ".18s" }}>
                FlexiLogic Africa is a technology company dedicated to empowering businesses across Africa through innovative, customized software solutions. Our mission is to deliver flexible, scalable, and client-focused systems that address the unique challenges of the African market.
              </p>
              <div className="afu flex gap-3 mb-12" style={{ animationDelay: ".26s" }}>
                <a href="#contact" className="btn-shimmer font-bold text-[14px] text-[#0B1221] border-none rounded-xl px-7 py-3 cursor-pointer flex items-center gap-2 no-underline">
                  Start a Project <Arr />
                </a>
                <button onClick={() => setShowreel(true)}
                  className="group font-semibold text-[14px] bg-transparent text-white border border-white/20 rounded-xl px-7 py-3 cursor-pointer flex items-center gap-2.5 hover:border-[#C8922A]/50 hover:bg-[#C8922A]/5 transition-all">
                  <span className="w-8 h-8 rounded-full bg-[#C8922A]/15 border border-[#C8922A]/30 flex items-center justify-center group-hover:bg-[#C8922A]/25 transition-all pl-0.5">
                    <PlayIcon sz={14} />
                  </span>
                  Watch Showreel
                </button>
            ...
            <div className="relative h-[500px]">
              <div className="afr af1 absolute top-0 left-[6%] w-[295px] bg-[#131D35] border border-white/[0.08] rounded-[18px] overflow-hidden shadow-[0_28px_70px_rgba(0,0,0,0.45)]" style={{ animationDelay: ".3s" }}>
                ...
                <div className="p-5">
                  {[{ n: "EduAdmin Pro", s: "LIVE", col: "#00C896", bg: "rgba(0,200,150,0.1)" }, { n: "EduVest Platform", s: "LIVE", col: "#00C896", bg: "rgba(0,200,150,0.1)" }, { n: "Zim Marketplace", s: "REVIEW", col: "#C8922A", bg: "rgba(200,146,42,0.12)" }, { n: "PayGo Mobile", s: "BUILD", col: "#7C9FFF", bg: "rgba(124,159,255,0.1)" }].map(p => (
              ...
              <div className="afr absolute bg-white rounded-[16px] p-4 border-l-[3px] border-[#C8922A] shadow-[0_12px_40px_rgba(0,0,0,0.2)]" style={{ top: "68%", left: "4%", width: 212, animationDelay: ".6s" }}>
                ...
                <span className="text-[10px] font-bold text-[#6B7592]">— CEO, EduVest Africa</span>
              </div>
        ...
        <svg className="absolute bottom-[-2px] left-0 right-0 w-full" viewBox="0 0 1440 60" preserveAspectRatio="none" height="60">
      ...
      <div className="relative overflow-hidden py-16 px-12" style={{ background: "linear-gradient(135deg,#0B1221 0%,#1B2847 100%)" }}>
        ...
        <div className="relative z-10 max-w-[1100px] mx-auto flex items-center justify-between gap-8">
          ...
          <button onClick={() => setShowreel(true)}
            className="group flex-shrink-0 flex items-center gap-4 bg-white/[0.06] hover:bg-[#C8922A]/10 border border-white/10 hover:border-[#C8922A]/40 rounded-2xl px-8 py-5 transition-all duration-300 cursor-pointer">
            <div className="w-14 h-14 rounded-full bg-[#C8922A] flex items-center justify-center shadow-[0_0_32px_rgba(200,146,42,0.4)] group-hover:scale-110 transition-transform pl-1">
              <PlayIcon sz={24} />
            </div>
            <div className="text-left">
              <div className="text-white font-bold text-[15px]">Watch Showreel</div>
              <div className="text-white/40 text-[12px]">2025 · Studio Reel</div>
            </div>
          </button>
        </div>
      </div>
      ...
      <footer className="border-t border-white/[0.07] pt-14 pb-8 px-12 bg-[#080F1E]">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="flex justify-between items-center">
            ...
            {/* <span className="text-[12px] text-white/25">Built in Zimbabwe 🇿🇼 · Serving the world 🌍</span> */}
          </div>
        </div>
      </footer>
    </>
